<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-0.5">Manage all registered users</p>
            </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-slate-800">{{ $verified }}</p>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex items-center gap-4">
            <div class="w-10 h-10 bg-amber-50 rounded-xl flex items-center justify-center flex-shrink-0">
                <svg class="w-5 h-5 text-amber-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
            <div>
                <p class="text-2xl font-bold text-slate-800">{{ $total - $verified }}</p>
        ...
        <div class="px-5 py-4 border-b border-slate-100">
            <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-col sm:flex-row gap-3">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('admin.users.index') }}" class="px-4 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 text-sm font-medium rounded-lg transition">
                        Clear
                    </a>
                @endif
            </form>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                        ...
                        <td class="px-5 py-3.5 text-slate-500 text-xs hidden sm:table-cell">
